...
$scenePath = "res://Scenes/mvp/mvp_test_map.tscn"
$logDirectory = Join-Path $projectRoot "debug_exports\logs"
$logFile = Join-Path $logDirectory "mvp_startup.log"
...
if ($exitCode -ne 0 -or $failureLines.Count -gt 0) {
    Write-Error "MVP startup check failed. Exit code: $exitCode. Matched lines: $($failureLines.Count)"
    exit 1
}

Write-Host "MVP_STARTUP_OK"
