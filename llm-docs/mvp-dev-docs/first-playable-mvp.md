...
拾荒猎犬只会从敌巢警戒范围内首次锁定目标。锁定后会持续追杀该目标；目标死亡后，猎犬优先尝试从自身附近续接其它目标，附近没有可续接目标时再从敌巢警戒范围内重新搜索，仍无目标时才返回敌巢。
...
第一版提供右下角独立“统计”菜单，默认显示最近 5 分钟窗口。菜单左侧先提供“资源”和“机器人”两个分类：

| 分类 | 指标 |
| --- | --- |
| 资源 | 每种资源的产出量、消耗量、净变化 |
| 机器人 | 每种蓝图机器人的生产量、损失量与损失原因、击杀量、规则触发总量、每条规则触发量、从未触发规则 |
